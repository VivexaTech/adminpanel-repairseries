...
/** Read `paymentDetails.upiDetails.upiId` — exact path; values normalized at call site. */
function readUpiIdFromPaymentDetails(paymentDetails) {
  if (!paymentDetails || typeof paymentDetails !== 'object') return ''
  const raw = paymentDetails?.upiDetails
...
function readBankDetails(paymentDetails) {
  if (!paymentDetails || typeof paymentDetails !== 'object') return null
  const b = paymentDetails.bankDetails
  if (!b || typeof b !== 'object') return null
  return {
    accountHolderName: b.accountHolderName != null ? String(b.accountHolderName) : '',
    accountNumber: b.accountNumber != null ? String(b.accountNumber) : '',
    bankName: b.bankName != null ? String(b.bankName) : '',
    ifscCode: b.ifscCode != null ? String(b.ifscCode) : '',
  }
}
...
export function TechnicianCard({
  ...
  const { recordTechnicianPayout, platformSettings } = useApp()
  const statusUi = useTechnicianLiveStatus(technician.id, technician.status)
  ...
  const [livePaymentDetails, setLivePaymentDetails] = useState(null)
  ...
  useEffect(() => {
    ...
    return subscribeDoc(
      ...
      (docRow) => {
        try {
          const pd = docRow?.paymentDetails
          setLivePaymentDetails(pd && typeof pd === 'object' ? pd : null)
        } catch {
          setLivePaymentDetails(null)
        }
      },
  ...
  const bankSafe = readBankDetails(livePaymentDetails)
  const hasBank = bankDetailsComplete(bankSafe)

  const techUpiNormalized = normalizeUpiId(readUpiIdFromPaymentDetails(livePaymentDetails))
  ...
  const payDetailsUpdated = useMemo(() => {
    const raw = livePaymentDetails?.updatedAt
  ...
  }, [livePaymentDetails?.updatedAt])

  return (
    <>
      <Card className="flex min-h-0 min-w-0 w-full max-w-none flex-col overflow-hidden p-0 shadow-[0_24px_60px_-28px_rgba(0,0,0,0.35)]">
        <div className="border-b border-[var(--outline-variant)]/50 bg-[color-mix(in_srgb,var(--primary)_6%,transparent)] px-5 py-6 sm:px-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-6">
            <div
              className="flex size-24 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[var(--primary)] to-[var(--primary-container)] text-2xl font-bold text-white shadow-lg ring-4 ring-white/10"
              aria-hidden
            >
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                <div className="min-w-0 space-y-1.5">
                  ...
                  <p className="text-sm leading-relaxed text-[var(--on-surface-variant)]">
                    <span className="text-[var(--on-surface)]/90">{categoryLabel || 'Category not set'} Technician</span>
                    {skillsLine ? (
                      <span className="block sm:mt-0.5 sm:inline sm:before:content-['·_']">{skillsLine}</span>
                    ) : null}
                  </p>
                </div>
                <Badge tone={statusUi.tone} className="w-fit shrink-0 whitespace-nowrap self-start">
